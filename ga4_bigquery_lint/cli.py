...
Usage:  ga4-bigquery-lint [path]      # a .sql file, or a directory of them
        ga4-bigquery-lint             # the SQL pack bundled in this package
        python3 verify_pack.py [path] # same thing from a clone
Exit code 0 = no errors, 1 = at least one error, 2 = nothing to check,
          3 = internal failure.
"""
...
# Two layouts must both work: an installed wheel, where the SQL ships inside the
# package as package data, and a checkout, where it may sit beside the package.
# Prefer the installed layout and fall back.
_QUERY_CANDIDATES = (
    os.path.join(_PACKAGE_DIR, "queries"),
    os.path.normpath(os.path.join(_PACKAGE_DIR, "..", "queries")),
)


def bundled_queries_dir():
    """Directory holding the SQL pack that ships with this package."""
    for candidate in _QUERY_CANDIDATES:
        if os.path.isdir(candidate):
            return candidate
    return _QUERY_CANDIDATES[-1]


def bundled_content_dir():
    """The default thing to check: the directory that holds `queries/`.

    In an installed wheel that is the package directory; in a checkout it is
    also the package directory, because the SQL lives inside the package. So
    the default run behaves identically whether or not the package is
    installed, which is the point of shipping the SQL as package data.
    """
    return os.path.dirname(bundled_queries_dir())


# Where the check reads from. main() overrides this from the command line.
CONTENT_DIR = bundled_content_dir()
...
EVENTS_TABLE_RE = re.compile(r"events_\*", re.IGNORECASE)
# BigQuery identifiers are case-insensitive, so `_table_suffix` and
# `_TABLE_SUFFIX` are the same column. Matching only the upper-case spelling
# reported "reads events_* with no _TABLE_SUFFIX filter -- unbounded scan" for a
# query that does filter on it whenever the author wrote the column lower-case.
SUFFIX_RE = re.compile(r"_TABLE_SUFFIX", re.IGNORECASE)
...
def collect_sql_files(target):
    """Return [(full_path, relative_path)] for a .sql file or a tree of them.

    A single file is accepted because the README documents
    `python3 verify_pack.py my-query.sql`. Before this, a file argument was
    walked with os.walk -- which yields nothing for a file -- so the tool
    reported "sql files found: 0" and exited 0, i.e. it silently checked
    nothing and claimed success.
    """
    ...
    for root, dirs, files in os.walk(target):
        ...
        for fn in sorted(files):
            if fn.lower().endswith(".sql"):
                ...
                out.append((full, os.path.relpath(full, target)))
    out.sort(key=lambda t: t[1])
    return out


def _unreadable_row(relpath, detail):
    """A result row for a file that could not be read at all.

    Without this, a binary or latin-1 .sql file raised UnicodeDecodeError out of
    check_file and killed the whole run with a traceback, so the user got no
    report for any of their other files either.
    """
    return {
        "path": relpath, "bytes": 0, "lines": 0, "statements": 0,
        "parsed": 0, "param_extractions": 0,
        "errors": ["could not read this file: %s" % detail],
        "warnings": [], "notes": [],
    }
...
def _parse_args(argv):
    ap = argparse.ArgumentParser(
        prog="ga4-bigquery-lint",
        description="Lint GA4 BigQuery SQL: a real BigQuery grammar parse plus "
                    "the event_params, _TABLE_SUFFIX, timestamp and cost checks "
                    "that catch queries which parse but return wrong numbers.",
        epilog="Exit codes: 0 no errors, 1 at least one error found, "
               "2 nothing to check, 3 internal failure.",
    )
    ap.add_argument("path", nargs="?", default=None, metavar="PATH",
                    help="a .sql file, or a directory to walk for *.sql. "
                         "Defaults to the SQL pack bundled in this package.")
    ap.add_argument("--report", metavar="PATH", default=None,
                    help="where to write the machine-readable JSON report. "
                         "Default: verification-report.json in the current "
                         "directory.")
    ap.add_argument("--version", action="version",
                    version="ga4-bigquery-lint 0.1.0")
...
def _run(argv=None):
    ...
    if args.path is None:
        ...
        if not (os.path.isfile(target) or os.path.isdir(target)):
            sys.stderr.write("error: %s is not a regular file or a directory\n"
                             % args.path)
            return 2

    CONTENT_DIR = target
    sql_files = collect_sql_files(target)
    if not sql_files:
        sys.stderr.write("error: no .sql files found in %s -- nothing was checked. "
                         "Pass a .sql file or a directory containing them.\n" % target)
        return 2
    ...
    rows = []
    for full, rel in sql_files:
        try:
            rows.append(check_file(full, rel))
        except UnicodeDecodeError as exc:
            rows.append(_unreadable_row(rel, "not valid UTF-8 text (%s)" % exc))
        except OSError as exc:
            rows.append(_unreadable_row(rel, "%s: %s" % (type(exc).__name__, exc)))
        except Exception as exc:  # one bad file must not lose the whole report
            rows.append(_unreadable_row(rel, "%s: %s" % (type(exc).__name__, exc)))
    ...
    # The report goes to the CURRENT directory by default, not next to the SQL.
    # When this defaulted to the content directory, an installed copy of the tool
    # tried to write verification-report.json into its own package directory
    # inside site-packages. Running from a checkout this is the same file as
    # before, so the documented workflow is unchanged.
    if args.report:
        out_path = os.path.abspath(args.report)
    else:
        out_path = os.path.join(os.getcwd(), "verification-report.json")
...
def main(argv=None):
    """Console-script entry point: never let an unexpected failure escape as a
    traceback, and always exit non-zero when something went wrong."""
    try:
        return _run(argv)
